...
def get_dataset(dir_paths, batch_size, num_bands=12, shuffle_buffer=200000, crop_season_only=False, predict_on_N_imgs=5, perc_field_fill=0.8,
    ...
    def image_selector_proc_fn(*input_val):
        ...
        #     Rsq_vals = tf.map_fn(linear_reg, tf.boolean_mask(unique_dates, crop_season_mask), fn_output_signature=(tf.float32,tf.float32,tf.float32))
        ...
        return band_vals_ret_map_out, gt_rsq, rsq_max
    ...
    if parametric:
        ds = ds.padded_batch(batch_size, padded_shapes=([None, 2*num_bands], [None], []), padding_values=-1.)
    else:
        ds = ds.padded_batch(batch_size, padded_shapes=([None, 5*num_bands], [None], []), padding_values=-1.)
